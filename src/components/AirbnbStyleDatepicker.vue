<template>
  <div class="airbnb-style-datepicker">
    <div class="form-group datepicker" :id="id" :class="triggerClasses" :style="triggerStyles" @click="toggleDatepicker">
      <div class="click-area" :style="triggerStyles">
        <div class="floating-label">{{labelText}}</div>
        <div class="selected-text" :class="{placeholder: !datesSelected}">{{selectedDateText}}</div>
        <span v-if="showError" class="validation-message">{{ error }}</span>
      </div>
    </div>
    <transition name="fade">
      <div id="datepicker-wrapper" class="datepicker-wrapper" v-if="showDatepicker" :class="wrapperClasses" :style="showFullscreen ? undefined : wrapperStyles" v-click-outside="closeDatepicker">
        <div class="modal-header mobile-only" v-if="showFullscreen">
          <div class="modal-close" @click="closeDatepicker"><ibe-icon :code="$icons.close"></ibe-icon></div>
          <h3>{{$translations.criteria.datesPlaceholder}}</h3>
        </div>
...
export default {
  ...
  methods: {
    ...
    getWeeks(date) {
      ...
      const skipDaysUntilFirstInMonth = this.days.findIndex(day => day === firstDayName)
      let weeks = []
      let week = []
